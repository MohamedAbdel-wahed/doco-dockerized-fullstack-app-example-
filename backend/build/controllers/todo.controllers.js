...
class TodoController {
    ...
    createTodo = async (req, res) => {
        ...
        res.status(http_status_codes_1.StatusCodes.CREATED).json({ todo: newTodo, msg: "Todo has been created!" });
    };
    // get all todos
    getTodos = async (req, res) => {
        ...
        res.status(http_status_codes_1.StatusCodes.OK).json({ todos, msg: "All Todos have been fetched!" });
    };
    //Get a single todo
    ...
    updateTodo = async (req, res) => {
        const { id } = req.params;
        const updatedTodo = await todo_models_1.Todo.findByIdAndUpdate({ _id: id }, req.body, { new: true });
        if (!updatedTodo) {
            throw new Error("Requested todo not found!");
        }
        res.status(http_status_codes_1.StatusCodes.OK).json({ todo: updatedTodo, msg: "Todo has been updated" });
    };
    // delete todo
    deleteTodo = async (req, res) => {
        ...
        res.status(http_status_codes_1.StatusCodes.OK).json({ todo: deletedTodo, msg: "Todo has been deleted" });
    };
}
